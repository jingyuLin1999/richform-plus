...
<template>
  <form :class="[
    ...
    <perfect-scrollbar :style="{ 'min-height': '20px' }" :ref="formId + '-scrollbar'">
      <!-- 顶部按钮 -->
      <actions v-if="showBtns" :actions="topActions" :isDesign="isDesign" :form="friendSchema"></actions>
      <!-- 核心布局 -->
      ...
      <!-- 底部按钮 -->
      <actions v-if="showBtns" :actions="buttomActions" :isDesign="isDesign" :form="friendSchema"></actions>
      <!-- 错误信息 -->
...
export default defineComponent({
  ...
  components: { FormLayout, PerfectScrollbar, Actions },
  props: {
  ...
  computed: {
    friendSchema() {
      ...
      for (let key in pickSpecSchema) {
        if (pickSpecSchema[key].length == 0) friendSchema.key = null;
      }
      return friendSchema;
    },
    friendForm() {
  ...
  methods: {
    ...
    onDesignClicked(clicked) {
      ...
      if (clicked == this.lastClicked) {
        ...
        this.lastClicked.activeDesign = false;

        // 设置当前点击事件
        clicked.isClicked = true;
        clicked.activeDesign = true;
      }
    ...
    onReset() {
      if (this.deepValues) {
        ...
        for (let key in this.values)
          this.values[key] = this.tofriendValue(this.values[key]);
    },
    // 全局校验
    globalValidate() {
      // 未传入schema，无法校验，直接返回
      let rawFriendSchema = toRaw(this.friendSchema);
      if (!Object.keys(rawFriendSchema).length) return true;
      // 开始校验
      this.fieldErrors = {};
      // 处理验证一次后，schem规则改变，再次验证错误信息还是保留第一次的
      // 故必须克隆一份改变地址，不然即使schema改变了也不会更新
      // https://ajv.js.org/api.html#api-validateschema
      let valid = AJV.validate(clone(rawFriendSchema), toRaw(this.values));
      if (!valid) {
