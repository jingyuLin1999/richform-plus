...
export default {
    methods: {
        ...
        deepSetValue(keys = [], obj = {}, value, notExistAddObj = false) {
            ...
            keys.map((key, index) => {
                if (notExistAddObj && !obj[key]) obj[key] = {}
                pickObj = obj[key];
                if (pickObj && keys.length != index + 1) obj = pickObj;
                if (obj && keys.length == index + 1) obj[key] = value;
            });
        },
        // 深度获取
        deepPick(keys = [], obj) {
            ...
            for (let index = 0; index < dicts.length; index++) {
                ...
                if (dictItem.keyValue == fieldValue || dictItem.keyValue == "any") {
                    ...
                    if (
                        ...
                        dictItem.field.options = options;
                    } else if (Array.isArray(dictItem.dictValue)) {
                        // 若是数组，则直接赋值给options
                        dictItem.field.options = dictItem.dictValue;
                        ...
                        dictItem.field.options = fieldValue ? filterOptions : dictItem.options;
                    }
